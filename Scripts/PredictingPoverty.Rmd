...
receta_cart <- recipe(
  ...
  step_dummy(all_nominal_predictors()) %>%  # 1️⃣ Convierte factores a dummies
  step_smote(Pobre) %>%                     # 2️⃣ Balancea con SMOTE
  step_center(all_predictors()) %>%         # 3️⃣ Centra variables numéricas
  step_scale(all_predictors())              # 4️⃣ Escala variables numéricas
...
# ---- Árbol podado ----
cp_optimo <- modelo_cart_smote$bestTune$cp
arbol_podado <- prune(modelo_cart_smote$finalModel, cp = cp_optimo)

prp(arbol_podado, under = TRUE, branch.lty = 2,
    yesno = 1, faclen = 0, varlen = 15,
    box.palette = "-RdYlGn", main = "Árbol CART Podado (con SMOTE)")

# ---- Predicciones ----
pred_prob <- predict(modelo_cart_smote, newdata = test_hogares_final, type = "prob")

test_hogares_final$Prob_Pobre <- pred_prob[, "Pobre"]
test_hogares_final$pobre <- ifelse(test_hogares_final$Prob_Pobre >= 0.5, 1, 0)

predictSample <- test_hogares_final |> select(id, pobre)
head(predictSample)

saveRDS(modelo_cart_smote, file = "CART_cp_0_004.rds")
write.csv(predictSample, "CART_cp_0_004.csv", row.names = FALSE)

cat("✅ Modelo guardado como 'modelo_cart_cv_002.rds'\n")
cat("✅ Predicciones guardadas como 'predicciones_CART_002.csv'\n")
...
# --- Entrenamiento del modelo CART con Gini ---
set.seed(123)
modelo_cart_cv <- train(
  ...
  method = "rpart",
  trControl = ctrl,
  parms = list(split = "gini"),     # 🔹 Criterio de impureza Gini
  control = rpart.control(maxdepth = 10, minsplit = 20, cp = 0.001),  # 🔹 Control de complejidad
...
# --- Visualización ---
prp(arbol_podado, under = TRUE, branch.lty = 2, yesno = 1,
    faclen = 0, varlen = 15, box.palette = "-RdYlGn",
    main = "Árbol CART Podado (Gini)")



# --- Predicción de probabilidades ---
test_hogares_final$Prob_Pobre <- predict(
  modelo_cart_cv, 
  newdata = test_hogares_final, 
  type = "prob"
)[, "Pobre"]

# --- Convertir a 0/1 usando umbral 0.5 ---
test_hogares_final$pobre <- ifelse(test_hogares_final$Prob_Pobre >= 0.5, 1, 0)
...
# --- Verificación ---
summary(predictSample$pobre)
sum(is.na(predictSample$pobre))  # Debe dar 0
nrow(predictSample) == nrow(test_hogares_final)  # Debe dar TRUE
head(predictSample)
...
set.seed(123)
modelo_cart_cv <- train(
  as.formula(paste("Pobre ~", paste(best_vars, collapse = " + "))),
  data = train_hogares_final,
  method = "rpart",
  trControl = ctrl,
...
prp(arbol_podado, under = TRUE, branch.lty = 2, yesno = 1,
    faclen = 0, varlen = 15, box.palette = "-RdYlGn",
    main = "Árbol CART Podado (Gini + RFE)")
...
# Alinear niveles
for(v in best_vars){
  if(is.factor(train_hogares_final[[v]])){
    test_hogares_final[[v]] <- factor(test_hogares_final[[v]], levels = levels(train_hogares_final[[v]]))
  }
}

# Predicción de probabilidad
test_hogares_final$Prob_Pobre <- predict(modelo_cart_cv, newdata = test_hogares_final, type = "prob")[, "Pobre"]
...
# Validaciones
sum(is.na(predictSample$pobre))      # Debe ser 0
nrow(predictSample) == nrow(test_hogares_final)  # Debe ser TRUE
head(predictSample)
```
...
```{r}
saveRDS(modelo_cart_cv, file = "modelo_cart_cv_002.rds")
write.csv(predictSample, "modelo_carT_002.csv", row.names = FALSE)

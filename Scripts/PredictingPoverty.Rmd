...
La base de datos de prueba definitiva para las predicciones de pobreza a nivel de Hogares es: test_hogares_final
...
preds_cart <- modelo_cart_1$pred


# Crear el objeto de predicción para ROC
pred_rocr_cart <- prediction(preds_cart$Pobre, preds_cart$obs)

# Calcular el F1-score a lo largo de todos los posibles umbrales
f1_cart_1 <- performance(pred_rocr, "f")

# Extraemos el índice del mejor F1
best_index_cart_1 <- which.max(slot(f1, "y.values")[[1]])

# Extraemos el valor de F1 Máximo
best_f1_cart_1 <- slot(f1, "y.values")[[1]][best_index_cart_1]
best_cutoff_cart_1 <- slot(f1, "x.values")[[1]][best_index_cart_1]
...
# Poda del arbol
cp_optimo_cart <- modelo_cart_1$bestTune$cp
arbol_prunin_cart <- prune(modelo_cart_1$finalModel, cp = cp_optimo_cart)
...
# Predicciones:
pred_cart_1<- predict(modelo_cart_1, newdata = test_hogares_final, type = "prob")

test_hogares_final$Prob_Pobre <- pred_cart_1[, "Pobre"] #Pobre es la clase positiva
test_hogares_final$pobre <- ifelse(test_hogares_final$Prob_Pobre >= best_cutoff_cart_1, 1, 0) 

Prediccion_cart_1 <- test_hogares_final |> select(id, pobre)
head(Prediccion_cart_1)
...
pred <- prediction(modelo_xgb_1$pred$Pobre, modelo_xgb_1$pred$obs)

# Calcular F1 en todos los posibles puntos de corte
perf <- performance(pred, "f")

# Encontrar el índice del F1 máximo
best_f1_index_xgb <- which.max(slot(perf, "y.values")[[1]])

# Extraer el cutoff correspondiente
best_cutoff_xgb <- slot(perf, "x.values")[[1]][best_f1_index_xgb]
best_f1_xgb <- slot(perf, "y.values")[[1]][best_f1_index_xgb]
...
pred_prob <- predict(modelo_xgb_1, newdata = test_hogares_final, type = "prob")
...
predict_boosting_xgb <- test_hogares_final %>%
  select(id, pobre)
...
#  Guardar modelo y predicciones
saveRDS(modelo_xgb_1, paste0(file_tag, ".rds"))
write.csv(predict_boosting_1, paste0(file_tag, ".csv"), row.names = FALSE)
...
receta_prep <- prep(receta_xgb) #entrena la receta
...
pred_cart_train <- predict(modelo_cart, newdata = train_ready, type = "prob")[, "Pobre"] #predicció cart
...
prob_ens_opt <- mejor_peso * pred_xgb_train + (1 - mejor_peso) * pred_cart_train

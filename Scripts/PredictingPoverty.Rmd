...
# Crear el objeto de predicción para ROCR
pred_rocr <- prediction(preds$Pobre, preds$obs)

# Calcular el F1-score a lo largo de todos los posibles umbrales
perf_f <- performance(pred_rocr, "f")

# Extraemos el índice del mejor F1
best_index <- which.max(slot(perf_f, "y.values")[[1]])

# Extraemos el valor de F1 Máximo
best_f1 <- slot(perf_f, "y.values")[[1]][best_index]
best_cutoff <- slot(perf_f, "x.values")[[1]][best_index]

cat("Mejor umbral (cutoff):", round(best_cutoff, 3), "\n")
cat("F1 máximo estimado:", round(best_f1, 3), "\n")


# Poda del arbol
cp_optimo <- modelo_cart$bestTune$cp
arbol_podado <- prune(modelo_cart$finalModel, cp = cp_optimo)
...
test_hogares_final$Prob_Pobre <- pred_prob[, "Pobre"] #Pobre es la clase positiva

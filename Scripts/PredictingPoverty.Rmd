...
train_hogares_final<- train_hogares_final%>%
  filter(!(Nivel_educativo %in% c("No sabe, no informa"))) %>%
  mutate(
    Nivel_educativo = case_when(
      Nivel_educativo == "Ninguno" ~ "Ninguno",
      Nivel_educativo %in% c("Preescolar", "Básica primaria", "Básica secundaria", "Media") ~ "Máximo bachiller",
      Nivel_educativo == "Superior o universitaria" ~ "Superior o universitaria",
      TRUE ~ NA_character_
    ),

...
```
Revision de datos faltantes en train y test hogares:
```{r}
...
```{r}
# Función para imputar NAs con la moda
...
# Imputar por moda en ambas variables y bases
train_hogares_final <- train_hogares_final %>%
  imputar_moda("Actividad_Ocup") %>%
  imputar_moda("Nivel_educativo")

test_hogares_final <- test_hogares_final %>%
  imputar_moda("Actividad_Ocup") %>%
  imputar_moda("Nivel_educativo")

# Convertir ambas variables a factores
train_hogares_final <- train_hogares_final %>%
  mutate(
    Actividad_Ocup = as.factor(Actividad_Ocup),
...
# Verificar resultados después de imputar
cat("DESPUÉS DE IMPUTAR:\n")
cat("Train - Actividad_Ocup:", sum(is.na(train_hogares_final$Actividad_Ocup)), "\n")
cat("Train - Nivel_educativo:", sum(is.na(train_hogares_final$Nivel_educativo)), "\n")
cat("Test  - Actividad_Ocup:", sum(is.na(test_hogares_final$Actividad_Ocup)), "\n")
cat("Test  - Nivel_educativo:", sum(is.na(test_hogares_final$Nivel_educativo)), "\n")
...
dicc_final <- tibble(
  ...
  mutate(
    Descripcion = case_when(
      ...
      Variable == "Dominio" ~ "Dominio geográfico: áreas metropolitanas, cabeceras y resto",
      Variable == "Tipo_vivienda" ~ "Tipo de vivienda: a. Propia pagada, b. Propia pagando, c. Arriendo/subarriendo, d. Usufructo, e. Ocupante sin título, f. Otra",
      Variable == "Nper" ~ "Número de personas en el hogar",
      Variable == "Depto" ~ "Departamento de residencia",
      Variable == "Pobre" ~ "1 = Pobre ; 0 = No pobre (solo en entrenamiento)",
...
dicc_final <- dicc_final %>%
  gt() %>%
  tab_header(
    title = "Diccionario de Variables: train_hogares_final"
  ) %>%
  cols_label(
    Variable = "Variable",
    Descripcion = "Descripción"
...
tabla_estadisticas <- resumen %>%
  gt() %>%
  tab_header(
    title = "Estadísticas Descriptivas: train_hogares_final"
  ) %>%
  fmt_number(

...
tipo_var_hogares <- function(df, nombre_df) {
...
# Crear diccionarios
var_train_hog <- tipo_var_hogares(train_hogares, "train")
var_test_hog  <- tipo_var_hogares(test_hogares, "test")
...
vars_unidas <- c(vars_comunes_hog, "Pobre")

# Filtro de train_hogares
train_hogares2 <- train_hogares %>%
  select(all_of(vars_unidas))
...
```{r}
# Renombrar variables en train_hogares2 (ya lo tienes hecho)
...
# Guardar tabla HTML y PDF
gtsave(tabla_gt, "diccionario_variables.html")
webshot("diccionario_variables.html", "diccionario_variables.pdf")
...
# Exportar a PDF
gtsave(tabla_estadisticas, "estadisticas_descriptivas.html")
webshot2::webshot("estadisticas_descriptivas.html", "estadisticas_descriptivas.pdf")
